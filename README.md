...
- **Scan phone** — find every `.epub` on the device (Android; first run asks for all-files access)
...
For a versioned file, tag `v5.0.0` and install from **Releases**. See [docs/RELEASE.md](docs/RELEASE.md). After 4.0.0, a newer versionCode signed with the same key updates the same LoreGuard and keeps your library. Builds through 4.1.4 used a new debug key each release, so Android blocked those updates; uninstall that copy once, then install 4.1.5 or later. Uninstall any 3.x build first; it was a different app id.
...
- Library search, labels, pin, Books / Audiobooks shelves
